...
class TestStateStore(unittest.TestCase):
    @staticmethod
    def _make_settings():
        return AppSettings.from_module(config)

    def _make_store(self, saved: dict | None = None):
        payload = saved if isinstance(saved, dict) else {}
        return ModeStateStore.from_saved(payload, settings=self._make_settings())
