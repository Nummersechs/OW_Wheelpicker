...
class DummyMainWindow:
    ...
    def _mode_for_state_capture(self) -> str:
        mode_to_capture = str(self.current_mode or "players")
        if mode_to_capture == "maps":
            fallback = str(self.last_non_hero_mode or "players")
            mode_to_capture = fallback if fallback in ("players", "heroes") else "players"
        return mode_to_capture

    def gather_state_snapshot(self):
        mode_to_capture = self._mode_for_state_capture()
        hero_ban_active = bool(self.hero_ban_active) if mode_to_capture == "heroes" else False
...
class DummyMainWindowWithSnapshot(DummyMainWindow):
    def __init__(self):
        super().__init__()
    ...
    def gather_state_snapshot(self):
        ...
        return {"volume": 99, "language": "de", "theme": "dark", "players": {}, "heroes": {}, "maps": {}}
...
class TestStateSnapshotBuilder(unittest.TestCase):
    ...
    def test_gather_state_uses_main_window_snapshot_provider_when_available(self):
        mw = DummyMainWindowWithSnapshot()
        builder = StateSnapshotBuilder(mw)

        state = builder.gather_state()

        self.assertEqual(mw.snapshot_calls, 1)
        self.assertEqual(state["volume"], 99)
        self.assertEqual(mw._state_store.capture_calls, [])
