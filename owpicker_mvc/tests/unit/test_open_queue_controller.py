...
class DummyWheel:
    ...
    def get_enabled_names(self):
        disabled = set(self._disabled_labels)
        result = []
    ...
    def get_subrole_labels(self):
        return list(self.subrole_labels)

    def is_pair_mode_enabled(self) -> bool:
        return bool(self.pair_mode)

    def set_pair_mode_enabled(self, enabled: bool) -> None:
        self.pair_mode = bool(enabled)

    def is_subrole_filter_enabled(self) -> bool:
        return bool(self.use_subrole_filter)

    def set_subrole_filter_enabled(self, enabled: bool) -> None:
        self.use_subrole_filter = bool(enabled)

    def set_included_in_global_spin(self, enabled: bool) -> None:
        self._selected = bool(enabled)

    def get_override_entries(self):
        return self._override_entries

    def get_disabled_indices(self):
        return set(self._disabled_indices)

    def restore_disabled_indices(self, indices):
        self._disabled_indices = set(indices or set())
