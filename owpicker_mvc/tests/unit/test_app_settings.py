...
class TestAppSettings(unittest.TestCase):
    ...
    def test_map_section_exposes_categories_and_list_height_controls(self):
        settings = AppSettings(
            values={
                "MAP_CATEGORIES": "Control, Escort, control,  Push ",
                "MAP_INCLUDE_DEFAULTS": "Push,Invalid,control",
                "MAP_LIST_NAMES_MIN_VISIBLE_ROWS": "3",
                "MAP_LIST_NAMES_MAX_VISIBLE_ROWS": "2",
                "MAP_LIST_NAMES_EXTRA_PADDING_PX": "-5",
            }
        )

        self.assertEqual(settings.map.categories, ("Control", "Escort", "Push"))
        self.assertEqual(settings.map.include_defaults, ("Push", "control"))
        self.assertEqual(settings.map.list_names_min_visible_rows, 3)
        self.assertEqual(settings.map.list_names_max_visible_rows, 3)
        self.assertEqual(settings.map.list_names_extra_padding_px, 0)
        self.assertEqual(settings.resolve("MAP_LIST_NAMES_MIN_VISIBLE_ROWS", 0), 3)
        self.assertEqual(settings.resolve("MAP_LIST_NAMES_EXTRA_PADDING_PX", 99), 0)
