...
def _set_stylesheet_if_needed(widget: QtWidgets.QWidget, style_key: str, style: str) -> None:
    if widget is None:
        return
...
def style_primary_button(btn: QtWidgets.QPushButton, theme: theme_util.Theme) -> None:
    if not btn:
        return
    _set_stylesheet_if_needed(btn, f"primary:{theme.key}", _primary_button_style(theme))


def style_include_button(btn: QtWidgets.QPushButton, theme: theme_util.Theme) -> None:
    if not btn:
        return
    _set_stylesheet_if_needed(btn, f"include:{theme.key}", _include_button_style(theme))


def style_danger_button(btn: QtWidgets.QPushButton, theme: theme_util.Theme) -> None:
    if not btn:
        return
    _set_stylesheet_if_needed(btn, f"danger:{theme.key}", _danger_button_style(theme))


def style_success_button(btn: QtWidgets.QPushButton, theme: theme_util.Theme) -> None:
    if not btn:
        return
    _set_stylesheet_if_needed(btn, f"success:{theme.key}", _success_button_style(theme))


def style_warning_button(btn: QtWidgets.QPushButton, theme: theme_util.Theme) -> None:
    if not btn:
        return
    _set_stylesheet_if_needed(btn, f"warning:{theme.key}", _warning_button_style(theme))


def style_names_list(list_widget: QtWidgets.QListWidget, theme: theme_util.Theme) -> None:
    if not list_widget:
        return
    _set_stylesheet_if_needed(list_widget, f"names_list:{theme.key}", _names_list_style(theme))
