...
_CONFIG_IDENTIFIER_HINTS_FALLBACK: tuple[str, ...] | None = None


def _fallback_config_identifier_hints() -> tuple[str, ...]:
    global _CONFIG_IDENTIFIER_HINTS_FALLBACK
    cached = _CONFIG_IDENTIFIER_HINTS_FALLBACK
    ...
    _CONFIG_IDENTIFIER_HINTS_FALLBACK = cached
    return cached
...
def _config_identifier_hints() -> tuple[str, ...]:
    ...
    if not isinstance(source, dict) or not source:
        return _fallback_config_identifier_hints()
    hints: set[str] = set()
    for key in source.keys():
        key_text = str(key or "")
        if not key_text.isupper():
