...
def _build_recall_retry_cfg(cfg: dict) -> dict:
    ...
    psm_values: list[int] = []
    _append_unique_ints(psm_values, [cfg.get("psm_primary", 6)])
    if bool(cfg.get("recall_retry_use_fallback_psm", True)):
        _append_unique_ints(psm_values, [cfg.get("psm_fallback", 11)])
    _append_unique_ints(psm_values, cfg.get("psm_values", ()))
    _append_unique_ints(psm_values, cfg.get("retry_extra_psm_values", ()))
